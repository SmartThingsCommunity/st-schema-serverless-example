...
function websocketKeepAlive() {
  ...
  webSocketTimerId = setTimeout(websocketKeepAlive, timeout)
}
...
function stateMapAttribute(stAttribute) {
  switch(stAttribute) {
    case 'level':
      return 'brightness'
  }
...
function connectWebsocket() {
  ...
  webSocket.onopen = function (event) {
    console.log(`Websocket onopen ${JSON.stringify(event)}`)
  }
...
$( document ).ready(function() {
  ...
  $.get('/devices/viewData', function(viewData) {
    ...
    if (viewData.websocketUrl) {
      webSocketUrl = viewData.websocketUrl
      connectWebsocket(viewData)
    }
  });
